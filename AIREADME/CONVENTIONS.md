...
- **简历文件命名**：根目录最新版 `MaxwellLi-<TargetRole>.{md,html}`（**不带日期**，常驻）；归档到 `versions/` 时才加 `-YYYYMMDD`。无 `Latest` 后缀（工程化味重，用户否决过）。当前 TargetRole = `AIProductManager`。
...
## 偏好模式
- **内容三处同步**：定位语 / 能力定位句 / 4 大独立项目 / 项目命名 任一变化 → 简历 MD + 简历 HTML + 主页(`site/data/home-data.js` → index.html) + README **四处都改**。
