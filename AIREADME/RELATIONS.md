...
| **worklog**（wiki 数据 + 简历源） | ① `build.js` 读 `wiki/projects/<wiki_slug>.md` 抽详情页 fact ② chat-api `loadResume()` 读 `wiki/job/me/resume/MaxwellLi-AIProductManager.md` 入 RAG | `../worklog/AIREADME/SPEC.md` |
...
- **简历源（2026-05-26 新增的第 2 处）**：chat-api `loadResume()`（`build-embeddings.mjs`）直读 `wiki/job/me/resume/MaxwellLi-AIProductManager.md` 入 RAG。走**主源通道**、不经 scan-sources 的 worklog walk，故 worklog 的 `wiki/job` 排除**不挡它**（无需白名单）。简历真相源现归 worklog（取代本仓库根的 `MaxwellLi-*`）。
