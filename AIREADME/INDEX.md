...
> 李越男 (Maxwell) 的个人主页：终端体静态站 maxwellii.com（V1 简介 / V2 LLM 化身对话）+ 简历多版本归档 + chat-api RAG 后端 ｜ 生命周期: active
...
| RELATIONS | ✅ | 出向：chat-api→newapi-proxy(LLM) + build→worklog(wiki 数据契约，唯一合法回读) |
| SPEC | ✅ | chat-api HTTP/SSE 契约 + 站点 URL 路由 + 详情页数据 frontmatter 契约 |
| ARCHITECTURE | ✅ | 3 组件（site / chat-api / 简历产物）+ RAG 数据流 + 禁改项 |
...
| CONVENTIONS | ✅ | kebab-case slug / 双名 frontmatter / 终端隐喻禁改 / 简历命名 / CSP 无 inline |
| DECISIONS | ✅ | 8 条 ADR（方案 C / RAG vault / 4 层隐私 / newapi 切换 / CHAT_LLM_* 等）|
...
- 改简历 / 主页文案 → CONVENTIONS（命名·同步三处）+ PRD（定位语）
